...
- Release builds now emit a first-layer ExeWrap installer `.exe` with a stored ZIP payload appended after the ExeWrap config end marker. The vendored ExeWrap 2.1.0 launcher carries an `asInvoker` manifest so Windows does not apply filename-based installer elevation heuristics. The bootstrap command uses PowerShell to extract itself with `tar.exe` into a random temp directory, converts ExeWrap's `@{args_as_json}` value to forward installer arguments safely into `bin\install.ps1`, and cleans up in `finally`; the generated installer then installs the payload into the configured install directory and writes an uninstall path when enabled.
