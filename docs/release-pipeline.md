...
Runtime creation and replacement are serialized per target path. A replacement is
built and validated in a unique sibling directory, then promoted with retries for
transient Windows access and sharing violations. If promotion fails after the old
runtime was moved aside, app-builder restores it before reporting the failure.
Generated Windows Python commands use launcher-relative interpreter paths. This
keeps pip-created commands valid across staging promotion; future pip installs
